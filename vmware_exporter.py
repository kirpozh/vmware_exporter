...
class VmwareCollector():
    ...
    @run_once_property
    @defer.inlineCallbacks
    def _tagCategoryIDs(self):
        """
        fetch a list of all category tags ids
        """
        session = yield self.session
        response = yield threads.deferToThread(
            session.get,
            'https://{host}/rest/com/vmware/cis/tagging/category'.format(host=self.host)
        )
        output = []
        try:
            output = response.json().get('value')
        except Exception as e:
            logging.error('Unable to fetch tag category IDs from vcenter {} ({})'.format(self.host, e))
    ...
    def _tagNames(self):
        ...
        for tagID in tagIDs:
            ...
            if tagObj:
                tagNames[tagObj.get('id')] = tagObj.get('name'), tagObj.get('category_id'))

        return tagNames

    @run_once_property
    @defer.inlineCallbacks
    def _tagCategoriesNames(self):
        """
        tag IDs are useless to enduser, so they have to be translated
        to the tag text
        """
        ...
        session = yield self.session
        tagCategoryIDs = yield self._tagCategoryIDs
        tagCategoriesNames = {}

        for tagcategoryID in tagCategoryIDs:
            response = yield threads.deferToThread(
                session.get,
                'https://{host}/rest/com/vmware/cis/tagging/category/id:{tagcategory_id}'.format(host=self.host,
                                                                                                 tagcategory_id=tagcategoryID)
            )
            tagObj = response.json().get('value', {})
            if tagObj:
                tagCategoriesNames[tagObj.get('id')] = tagObj.get('name')

        return tagCategoriesNames
    ...
    def tags(self):
        ...
        #tagCategories = yield self._tagCategoriesNames

        all_tags = []
        session = yield self.session
        cats = yield threads.deferToThread(
            session.get,
            f'https://{self.host}/rest/com/vmware/cis/tagging/category'
        )
        cats = cats.json()['value']
        for cat_id in cats:
            tags_in_cat = yield threads.deferToThread(
                session.post,
                f'https://{self.host}/rest/com/vmware/cis/tagging/tag/id:{cat_id}?~action=list-tags-for-category',
                json={"category_id": cat_id}
            )
            cat = yield threads.deferToThread(
                session.get,
                f'https://{self.host}/rest/com/vmware/cis/tagging/category/id:{cat_id}'
            )
            tags_in_cat = tags_in_cat.json()['value']
            cat = cat.json()['value']
            for tag_id in tags_in_cat:
                tag = yield threads.deferToThread(
                    session.get,
                    f'https://{self.host}/rest/com/vmware/cis/tagging/tag/id:{tag_id}'
                )
                tag = tag.json()['value']
                all_tags.append({
                    'id': tag_id,
        ...
        for tag in all_tags:
            if tag['category_name'] in ['CostCenterCode', 'AS', 'Environment']:
                att_objects = yield threads.deferToThread(
                    session.post,
                    f'https://{self.host}/rest/com/vmware/cis/tagging/tag-association/id:{tag["id"]}?~action=list-attached-objects',
                    json={"tag_id": tag["id"]}
                )
                att_objects = att_objects.json()['value']
                for obj in att_objects:
                    section = sections.get(obj.get('type'), 'others')
                    if section != "vms":
                        continue

                    if obj.get('id') not in tags[section]:
                        tags[section][obj.get('id')] = [(tag['name'], tag['category_name'])]
                    else:
                        tags[section][obj.get('id')].append((tag['name'], tag['category_name']))
        ...
        #     tagName, tagCategory = tagNames.get(attachedObj.get('tag_id'))
        #
        #     tagCategoryName = tagCategories.get(tagCategory)
        #
        #     if tagCategoryName not in ['CostCenterCode', 'AS', 'Environment']:
        #         continue
        #
        ...
        #
        #         if obj.get('id') not in tags[section]:
        #             tags[section][obj.get('id')] = [(tagName,tagCategoryName)]
        #         else:
        #             tags[section][obj.get('id')].append((tagName,tagCategoryName))
        #
        # fetch_time = datetime.datetime.utcnow() - start
        # logging.info("Fetched tags ({fetch_time})".format(fetch_time=fetch_time))
        #
